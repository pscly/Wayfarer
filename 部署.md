...
- 日志路径出现 `/home/runner/...`：这是 GitHub Actions runner 的目录，属于正常现象。
- `Pinned known_hosts missing entry ...`：pinned `known_hosts` 缺条目；更新 `secrets.KNOWN_HOSTS` 或提交 `.github/known_hosts`。
- `Permission denied (publickey,password)`：服务器未接受私钥；优先对齐 Actions 输出的 key fingerprint 与服务器 `authorized_keys`，并查看 `journalctl -u ssh`。

更完整的 CI/CD 排障说明请以 `README.md` 的 “CI/CD（GitHub Actions）” 章节为准。
