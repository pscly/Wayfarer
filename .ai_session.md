...
## [2026-02-04 08:49] - 修复：deploy 状态评论失败不影响 deploy 结论
- 原始需求：在部分仓库中 Actions 的 workflow token 可能被限制为只读，导致 `Report deploy status (issue #1)` 评论 POST 失败，从而误把 deploy job 判为失败。
- 实现目标/逻辑：在 `.github/workflows/cicd.yml` 的 `Report deploy status (issue #1)` step 上增加 `continue-on-error: true`，并保留 `if: always()`，确保“上报失败”不会影响 deploy job 的最终结论。
- 核心变更：
  - 修改：`.github/workflows/cicd.yml`
- 遗留/约束：无
